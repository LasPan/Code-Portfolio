...
public class OrbitCamera : MonoBehaviour
{
    ...
    bool AutomaticRotation()
    {
        ...
        Vector3 alignDelta = Quaternion.Inverse(gravityAlignment) * (focusPoint - previousFocusPoint);

        //Vector2 movement = new Vector2(focusPoint.x - previousFocusPoint.x,focusPoint.z - previousFocusPoint.z);
        Vector2 movement = new Vector2(alignDelta.x, alignDelta.z);
    ...
    void UpdateGravityAlignement()
    {
        ...
        {
            //gravityAlignment = Quaternion.Slerp(gravityAlignment, newAlignment, maxAngle / angle);  //As we've already made sure that we only interpolate when necessary the interpolator is 
                                                                                    //guaranteed to lie within the 0–1 range, so we can use SlerpUnclamped instead, avoiding unnecessary extra clamping.
